...
tPats = list(set([i.split("-")[1].split("_")[-1] for i in filesLog]))
tPat = tPats[0]
for tPat in tPats:
    (flogs, flnds) = (
        [flog for flog in filesLog if tPat in flog],
        [flnd for flnd in filesLnd if tPat in flnd]
    )
    logs = [pd.read_csv(f) for f in flogs]
    lnds = [
        srv.loadLandscape(
    ...
    fig.savefig(
        path.join(paths['data'], CODE, fNameBase[:-4]+'_'+tPat+'-KER.png'), 
    ...
    # ax.spines['left'].set_visible(False)
    fig.savefig(
        path.join(paths['data'], CODE, fNameBase[:-4]+'_'+tPat+'-GAV.png'), 
        facecolor=None, bbox_inches='tight', transparent=True,
        pad_inches=0, dpi=350
    )
    plt.close('all')
    ...
    fig.savefig(
        path.join(paths['data'], CODE, fNameBase[:-4]+'_'+tPat+'_OPT'),
